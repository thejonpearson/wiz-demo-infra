...
#     public IP addresses via curl that were hanging until timeout
...
#BACKUP_BUCKET=$(aws ssm get-parameter --name "${BUCKET_SSM_ID}" --query "Parameter.Value" --output text --region us-west-2 --with-decryption)
